...
JavaScript программы должны храниться в файлах с расширением .js.
...
<script src="filename.js"></script> тэги должны быть размещены ближе к концу документа на сколько это возможно. Это уменьшает влияние задержек загрузки сценария на другие компоненты страницы.
...
<h2>5. Комментарии</h2>
Комментарии должны быть в формате PHPDoc.
...
Все переменные должны быть объявлены перед использованием. JavaScript не требует этого, но при этом делает программу проще для чтения. Так же легче будет обнаружить необъявленных переменные. Использование глобальных переменных должны быть сведены к минимуму.
...
```
Если литерал функциии является анонимным, то должен быть один пробел между служебным словом и левой скобкой. Если пространство опущен, то может визульно показаться, что имя функции является <code>function</code>.
...
Когда функция должна быть вызвана немедленно, все выражение вызова должно быть завернуты в скобки, чтоб быдл очевидно, что возвращаемая величина является результатом функции, а не сама функция.
...
Имена должны быть сформированы из 26 прописных и строчных букв (A..Z, a..z), 10 цифр (0..9) и _(подчеркивание). Избегайте использования международных символов. Не используйте $ (знак доллара) или \ (обратная косая черта) в именах.
...
Переменные и функций должны начинаться с прописной буквы.
...
<ul>
    ...
    <li>Каждая <code>;</code> (точка с запятой) в объявлении <code>ащк</code> следует отделять пробелом.</li>
    <li>Пробелы должны следовать за каждой <code>,</code> (запятой).</li>
</ul>

<h2>11. Используйте JSLint</h2>
Весь код на JavaScript должен проходить проверку инструментом JSLint с такими настройками:
<ul>
...
Примечание!
Обявить глобальную переменную для JSLint можно так:
```javascript
/* global $, jQuery */
```
