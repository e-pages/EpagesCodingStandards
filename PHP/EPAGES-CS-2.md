...
Нужно стараться избегать строк длиной более 120 символов. Если строка превышает этот размер, то рекомендуется использовать правила переноса строки.

<h3>2.2. Правила переноса строки</h3>
Если длина строки превышает 120 символов, то рекомендуется пользоваться следующими правилами переноса:
<ul>
...
и открывающую фигурную НЕОБХОДИМО располагать на одной отдельной строке,
открывающую фигурную скобку СЛЕДУЕТ перенести на следующую строку.
...
<ul>
	<li>если тела всех частей иправляющие структуры состоят не более чем из одного выражения, то инструкция все равно ДОЛЖНА иметь фигурные скобки</li>
</ul>
```php
<?php
if (условие) 
...
?>
```
При написании инструкций ДОЛЖНО строго применяться правило "2.4. Форматирование подчиненности", тело иправляющие структуры ДОЛЖНО быть сдвинуто на один отступ вправо от самой иправляющие структуры. Фигурные скобки ДОЛЖНЫ находиться на отдельных строках и ДОЛЖНЫ быть на одном уровне с инструкцией.
...
<h3>3.3. Сложные иправляющие структуры</h3>
Сложные иправляющие структуры СЛЕДУЕТ разбивать по строкам в соответствии с правилами пункта 3.2. (Управляющие структуры if, else, while и т.п.) и 2.2. (Правила переноса строки)
...
Очень сложные иправляющие структуры рекомендуется разбивать на несколько более простых.
